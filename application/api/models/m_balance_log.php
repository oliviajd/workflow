...
/**
 * Description of m_balance_log
 *
 * @author lsk
 */
class m_balance_log extends CI_Model implements ObjInterface {
    ...
    public function add($data) {
        ...
        $this->db->insert(TABLE_BALANCE_LOG, $param);
    ...
    public function update($id, $data) {
        ...
        $this->db->update(TABLE_BALANCE_LOG, $param, array('id' => $id));
    ...
    public function detail($id) {
        ...
        if (empty($detail)) {
            ...
            $detail['status'] = $this->get_balance_log_status($detail['status']);
            return new obj_balance_log($detail);
        }
    }
    ...
    public function lists($condition, $page, $size, $order) {
        ...
        $this->db->select(TABLE_BALANCE_LOG . '.*');
        $this->_condition($condition);
        $rows = $this->db->get_where(TABLE_BALANCE_LOG)->result_array();
        foreach ($rows as $k => $v) {
            $v['status'] = $this->get_balance_log_status($v['status']);
            $rows[$k] = new obj_balance_log($v);
        }
        return $rows;
    }
    ...
    public function count($condition) {
        ...
        $this->_condition($condition);
        return $this->db->get_where(TABLE_BALANCE_LOG)->row(0)->count;
    }
    ...
    private function _detail($user_id) {
        $detail = $this->db->get_where(TABLE_BALANCE_LOG, array('user_id' => $user_id))->row_array();
        return empty($detail) ? false : $detail;
    }
    
    public function get_balance_log_status($key = false) {
        $data = array(
            0 => '未对账',
            1 => '对账完成',
            2 => '对账中',
    ...
    public function get($condition,$order) {
        ...
        $this->_condition($condition);
        $this->db->limit(1);
        $r = $this->db->get_where(TABLE_BALANCE_LOG)->row_array();
        return $r;
    }
