...
function funcAjaxAddField(page_field, d, cate_name)
{
    ...
    $.ajax({
        type : 'GET',
        url : '/api_admin/ajax?m=field_update',
        async : false,
        data : d,
        dataType : 'json',
        success : function(data){
            ...
            if(data.no == 1)
            {
                var html = ' <tr class="obj_field_'+d.field_id+' obj_list_tr">'
                   ...
                   +' <td>'+d.description+'</td>'
                   +' <td>'+d.sort+'</td>'
                   +' <td> <span style="padding-left:10px;"> <a href="/api_admin/obj_detail_update?cate_type='+cate_type+'&item_id='+item_id+'&field_id='+d.field_id+'" >修改</a> </span>'
...
function func_field_delete(field_id)
{
    if(confirm('你确定要删除吗?'))
    {
        $.ajax({
            type: 'GET',
            url : '/api_admin/ajax?m=field_delete',
            async : false,
...
<div class="container grid-s165m0 layout">
 <?php if(isset($_COOKIE['type'])  && $_COOKIE['type'] == 'admin'){?>
	<div class="sub_menu" style="padding: 10px;">
		...
		<!--<span><a href="/api_admin/category_update?cate_type=<?php echo $_REQUEST['cate_type']?>" >添加分类</a></span>-->
		...
		<tbody>
            <?php if(!empty($api_obj_detail_list)){?>
				...
				<span style="padding-left: 10px;"> <a
						href="/api_admin/obj_detail_update?cate_type=<?php echo $_REQUEST['cate_type']?>&item_id=<?php echo $_REQUEST['item_id']?>&field_id=<?php echo $v['field_id'];?>"
						class="btn">修改</a>
